...
| Dashboard Screen | App hub screen. | `app/src/main/java/com/pcosina/app/ui/screens/DashboardRefinedScreen.kt` | Main entry point for actions. | What is the main hub? | The dashboard with plan and feature shortcuts. |
| Meal Plan Screen | Weekly plan view. | `app/src/main/java/com/pcosina/app/ui/screens/MealPlanRefinedScreen.kt` | Displays generated plan. | Where do users see the plan? | On the Meal Plan tab. |
| Grocery List Screen | Consolidated ingredients view. | `app/src/main/java/com/pcosina/app/ui/screens/GroceryRefinedScreen.kt` | Reduces waste and guides shopping. | How is grocery list created? | Aggregated from chosen meals. |
...
| Progress Screen | Tracking and journal UI. | `app/src/main/java/com/pcosina/app/ui/screens/ProgressRefinedScreen.kt` | Logs adherence and reflections. | What is tracked? | Adherence, weight, and journaling. |
...
| UseCase | Domain logic extracted into dedicated classes. | `app/src/main/java/com/pcosina/app/domain/MealLoggingPolicyUseCase.kt`, `ProgressSummaryUseCase.kt`, `GroceryRebuildUseCase.kt`, `PlannerProfilePreparationUseCase.kt` | Keeps business rules outside screens and ViewModels. | Where is domain logic now? | Core app rules are in dedicated use cases, while plan generation itself still runs on the backend. |
